...
!$OMP&             SCHEDULE(STATIC,jblk)
        ...
        write (lp,*) 
        if     (forcw0) then
          write (lp,*) ' forcing constant in time between inputs'
        else
          write (lp,*) ' forcing linear   in time between inputs'
        endif
        write (lp,*) 
...
c
c --- msl pressure uses the halo, and is smoothed
c
      if     (iunit.eq.899) then
        call psmooth(field(1-nbdy,1-nbdy,2),0,1, ip, util1)
      endif
c
c --- wind stress uses the halo.
c
